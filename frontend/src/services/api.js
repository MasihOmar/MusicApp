...
const getDefaultApiUrl = () => {
  // For physical devices (both Android and iOS)
...
export const checkServerReachability = async () => {
  try {
    ...
    try {
      console.log('Making test request to:', `${API_URL}/playlists`);
...
export const songService = {
  ...
  getSongById: async (id) => {
    const response = await apiRequest(`/songs/${id}`);
    console.log('Raw API response for song:', JSON.stringify(response, null, 2));
    return response;
  },
  addSong: (songData) => apiRequest('/songs/add', 'POST', songData),
};

export const playlistService = {
  getAllPlaylists: () => apiRequest('/playlists'),
  getPlaylistById: (id) => apiRequest(`/playlists/${id}`),
  createPlaylist: (playlistData) => apiRequest('/playlists/add', 'POST', playlistData),
  getPlaylistSongs: (id) => apiRequest(`/playlists/${id}/songs`),
  addSongToPlaylist: (playlistId, songId) => 
    apiRequest(`/playlists/add-song/playlist/${playlistId}/song/${songId}`, 'POST'),
